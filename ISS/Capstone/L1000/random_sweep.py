...
def compute_auroc(y_true, prob, n_classes, avg):
    # binary
    if n_classes == 2:
        return roc_auc_score(y_true, prob[:, 1])
    # multiclass: IMPORTANT -> fix missing classes in y_true fold
...
def main():
    ...
    ap.add_argument("--out_dir", default="./results/Jiwoo/Random")
    ...
    ap.add_argument("--random_trials", type=int, default=5,
                    help="random gene draw repetitions (only used when gene_mode=random)")
    ...
    # helper: pick GE genes for this TOP_N and trial
    def pick_ge_genes(TOP_N, trial_idx):
        if TOP_N == "all":
        ...
        if args.gene_mode == "ranked":
            ...
            rng = np.random.default_rng(args.random_seed + 1000 * trial_idx + int(TOP_N))
    ...
    n_trials = args.random_trials if args.gene_mode == "random" else 1

    for TOP_N in TOP_N_LIST:
        trial_metrics = []  # list of dict: modality -> (acc_mean, f1_mean, auroc_mean)

        for trial in range(n_trials):
            available, topn_tag = pick_ge_genes(TOP_N, trial)

            if len(available) == 0:
                print(f"[WARN] TOP_N={TOP_N} trial={trial}: no matched genes, skip")
                continue
            ...
            fold_acc = {"CP": [], "GE": [], "Early Fusion": [], "Late Fusion": []}
            fold_f1  = {"CP": [], "GE": [], "Early Fusion": [], "Late Fusion": []}
            fold_auc = {"CP": [], "GE": [], "Early Fusion": [], "Late Fusion": []}
            ...
            for fold, (tr, te) in enumerate(sgkf.split(X_early, y_all, groups=groups), start=1):
                ...
                base_seed = args.seed + trial * 1000 + fold * 10
                ...
                for name, pred_, prob_ in [
                    ("CP", pred_cp, prob_cp),
                    ("GE", pred_ge, prob_ge),
                    ("Early Fusion", pred_ef, prob_ef),
                    ("Late Fusion", pred_lf, prob_lf),
                ]:
                    fold_acc[name].append(accuracy_score(y_te, pred_))
                    fold_f1[name].append(f1_score(y_te, pred_, average=avg))
                    fold_auc[name].append(compute_auroc(y_te, prob_, n_classes, avg))

                pred_dumps.append(pd.DataFrame({
            ...
            # fold 평균을 trial 결과로 저장
            trial_metrics.append({
                m: (
                    float(np.mean(fold_acc[m])),
                    float(np.mean(fold_f1[m])),
                    float(np.mean(fold_auc[m])),
                )
                for m in ["CP", "GE", "Early Fusion", "Late Fusion"]
            })

            print(f"[DONE] TOP_N={topn_tag} trial={trial} (genes used={len(available)})")

        # TOP_N별 trial mean/std 저장
        if len(trial_metrics) == 0:
            continue

        for modality in ["CP", "GE", "Early Fusion", "Late Fusion"]:
            accs = [tm[modality][0] for tm in trial_metrics]
            f1s  = [tm[modality][1] for tm in trial_metrics]
            aucs = [tm[modality][2] for tm in trial_metrics]
            ...
            summary_rows.append(dict(
                ...
                auroc_std=float(np.std(aucs, ddof=0)),
            ))

    res = pd.DataFrame(summary_rows)
    pred = pd.concat(pred_dumps, ignore_index=True) if len(pred_dumps) else pd.DataFrame()

    tag = f"{args.dataset}_torch_mlp_topn_{args.gene_mode}"
    if args.gene_mode == "random":
        tag += f"_trials{args.random_trials}_seed{args.random_seed}"
